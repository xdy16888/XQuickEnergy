            ...
            if ("SUCCESS".equals(jo.getString("resultCode"))) {
                ...
                if ("AVAILABLE".equals(applyAction)) {
                    if (currentEnergy >= jo.getInt("energy")) {
                        ...
                        Log.forest("领保护地[" + jo.getString("projectName") + "]#能量不足停止申请");
                        return false;
                    }
                } else {
                    Log.forest("领保护地[" + jo.getString("projectName") + "]#似乎没有了");
                    return false;
                }
            } else {
            ...
            if ("SUCCESS".equals(jo.getString("resultCode"))) {
                ...
                if ("AVAILABLE".equals(applyAction)) {
                    if (currentEnergy >= jo.getInt("energy")) {
                        ...
                        Log.forest("保护海洋[" + jo.getString("cultivationName") + "]#能量不足停止申请");
                    }
                } else {
                    Log.forest("保护海洋[" + jo.getString("cultivationName") + "]#似乎没有了");
                }
            } else {
                Log.recordLog(jo.getString("resultDesc"), s);
            }
        ...
        try {
            ...
            for (int applyCount = 1; applyCount <= count; applyCount++) {
                ...
                if ("SUCCESS".equals(jo.getString("resultCode"))) {
                    ...
                    String str = "保护海洋[" + itemName + "]#第" + appliedTimes + "次"
                            + "-获得奖励" + award;
                    Log.forest(str);
                } else {
                    Log.recordLog(jo.getString("resultDesc"), jo.toString());
                    Log.forest("保护海洋[" + itemName + "]#发生未知错误，停止申请");
                    break;
                }
                Thread.sleep(300);
